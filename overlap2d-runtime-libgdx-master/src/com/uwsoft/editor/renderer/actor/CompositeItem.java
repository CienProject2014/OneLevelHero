...
/**
 * ContainerActor that can have other children inside Also used as the root
 * actor for every scene Sort of a Group with extended capabilities for
 * searching items by id specified from editor attaching IScripts to items and
 * much more You are not encouraged to create instances of this, it is solely
 * tob e instantiated by Overlap2DRuntime inner workings
 *
...
public class CompositeItem extends Group implements IBaseItem {
	...
	private Comparator<IBaseItem> ZIndexComparator = new Comparator<IBaseItem>() {
		...
		public int compare(IBaseItem obj1, IBaseItem obj2) {
			return obj1.getLayerIndex() == obj2.getLayerIndex() ? obj1.getDataVO().zIndex - obj2.getDataVO().zIndex
					: obj1.getLayerIndex() - obj2.getLayerIndex();
			// return obj1.getLayerIndex() == obj2.getLayerIndex()?
			// ((Actor)obj1).getZIndex()-((Actor)obj2).getZIndex() :
			// obj1.getLayerIndex() - obj2.getLayerIndex();
		}
	};
	...
	/**
	 *
	 * @return CompositeItem parent of this Composite If this is a root item in
	 *         scene, returns null
	 */
	...
	/**
	 * Ads array of scripts to all children composite items that are all
	 * instances of same prefab library name
	 *
	 * @param name
	 *            - String library name
	 * @param iScripts
	...
	/**
	 * Ads iScript instance to all children composite items that are all
	 * instances of same prefab library name
	 *
	 * @param name
	 *            - String library name
	...
	/**
	 * Creates instances of provided iScript class for each item that is found
	 * in children composite items with the name equal to the library name they
	 * have been all instantiated from as a prefab uses libGDX Reflection
	 *
	 * @param name
	 *            - String library name
	 * @param iScript
	...
	/**
	 * Removes provided iScripts from all the children items with the name equal
	 * to the library name this items were instantiated from
	 * 
	 * @param name
	 * @param iScripts
	 */
	...
	private void reAssemble() {
		...
		for (int i = 0; i < dataVO.composite.sSelectBoxes.size(); i++) {
			...
			SelectBoxItem itm = new SelectBoxItem(tmpVo, essentials, this); // TODO
																			// need
																			// to
																			// fix
																			// this
	 ...
	 * @param itemName
	 *            - String name of library item that searchable children have
	 *            been instance of
	 * @return
	 */
	...
	private ArrayList<CompositeItem> findItemsByName(String itemName, CompositeItem itm,
			ArrayList<CompositeItem> recursiveArray) {
	...
	/**
	 * Ads item inside the composite and reassebmles everything It is not
	 * reccommended for general use, it is better to have items already there
	 * from editor But if you need dynamic one, then it has to be IBaseItem
	 **/
	public void addItem(IBaseItem item) {
		...
		if (item.getDataVO().physicsBodyData != null && item.getDataVO().meshId >= 0) {
			...
			item.setBody(PhysicsBodyLoader.createBody(essentials.world, item.getDataVO().physicsBodyData,
					essentials.rm.getProjectVO().meshes.get(item.getDataVO().meshId), new Vector2(mulX, mulY)));
	...
	/**
	 * If changes are made in layer ordering then this method call will
	 * reassemble all zIndexes to take actual effect
	 */
	...
	private void assemblePhysics() {
		...
		for (IBaseItem item : items) {
			...
			item.setBody(PhysicsBodyLoader.createBody(essentials.world, bodyData,
					essentials.rm.getProjectVO().meshes.get(itemVO.meshId + ""), mulVec));
	 ...
	 * Everything will chaneg in size, coordinates will be recalculated
	 * accordingly
	 *
	...
	public void applyResolution(String resolutionName) {
		if (resolutionName != null && !resolutionName.isEmpty()) {
			...
			if (curResolution != null) {
				float mulX = (float) curResolution.width
						/ (float) essentials.rm.getProjectVO().originalResolution.width;
	...
	public void act(float delta) {
		...
		if (essentials.world != null && body != null && dataVO.physicsBodyData != null
				&& dataVO.physicsBodyData.bodyType > 0 && !essentials.physicsStopped) {
	...
	public void setScissors(float x, float y, float w, float h) {
		...
		scissorBounds = new Rectangle(dataVO.scissorX * mulX, dataVO.scissorY * mulY, dataVO.scissorWidth * mulX,
				dataVO.scissorHeight * mulY);
	}
