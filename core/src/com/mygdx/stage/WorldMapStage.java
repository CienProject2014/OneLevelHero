...
public class WorldMapStage extends RootOverlap2DStage {
	@Autowired
	...
	public Stage makeStage() {
		...
		currentNode = sceneLoader.getRoot().getImageById(positionInfo.getCurrentNode());// 카메라 위치를 현재노드로 잡기 위하여 가져옴
		...
		Map<String, Connection> connectionMap = assets.worldNodeInfoMap.get(currentNode).getConnection();
		for (final Entry<String, Connection> connection : connectionMap.entrySet()) {
	...
	public void setCamera() {
		...
		float xvalue = this.getCurrent().getX() - assets.windowWidth / 2, yvalue = this.getCurrent().getY() - assets.windowHeight / 2;
		// x값이 오른쪽으로 벗어날 경우
