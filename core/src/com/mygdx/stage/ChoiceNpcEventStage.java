...
public class ChoiceNpcEventStage extends BaseOneLevelStage {
	@Autowired
	...
	private void addListener() {
		for (int i = 0; i < EVENT_SIZE; i++) {
			...
			if (eventManager.isEventOpen(selectedEvent)) {
				...
				choiceNpcEventListener.setIndex(i + 1);
				chatButtons.get(i).addListener(choiceNpcEventListener);
			} else {
	...
	private void showEventButton() {
		for (int i = 0; i < EVENT_SIZE; i++) {
			...
			chatButtons.add(new TextButton(eventManager.getCurrentNpc().getEvent(i + 1).getEventTitle(), chatStyles
					.get(i)));
		}
	}
}
