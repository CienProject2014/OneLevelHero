...
public class LoadLauncher {
	...
	public void setStatus() {
		...
		statusLoader = new StatusLoader(
				OneLevelHero.currentManager.getVersion(), "Hero", status_json,
				bag_json); // statusLoader에서 jsonObject를
							// 파싱하고 클래스 정보를 주입함
		unit = new Unit(statusLoader.getStatus(), statusLoader.getBag()); // unit클래스에
																			...
																			// 정보를
																			// 뿌려줌
		OneLevelHero.currentManager.setUnit(unit); // currentManager가 unit을
													// 소유하도록 만든다.
	}
}
