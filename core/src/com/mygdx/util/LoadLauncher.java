...
public class LoadLauncher {
	...
	public void setStatus() {
		...
		statusLoader = new StatusLoader(CurrentManager.getInstance()
				.getVersion(), "Hero", status_new_left, bag_json);

		unitLeft = new Hero("YongSa", statusLoader.getStatus(),
				statusLoader.getBag()); // unit클래스에 파싱한 json 정보를 뿌려줌
		CurrentManager.getInstance().setUnit(unitLeft); // currentManager가 unit을
														// 소유하도록
		// 만든다.
	}
}
