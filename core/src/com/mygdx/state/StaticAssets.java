...
public class StaticAssets {
	...
	public static TextureRegionDrawable bartexture_bg = new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("texture/bgcolour.png")), 50, 50));
	public static ProgressBarStyle barstyle_hp = new ProgressBarStyle(bartexture_bg, new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("texture/hpcolour.png")), 50, 50)));
	public static ProgressBarStyle barstyle_exp = new ProgressBarStyle(bartexture_bg, new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("texture/expcolour.png")), 50, 50)));
	public static ProgressBarStyle barstyle_turn = new ProgressBarStyle(bartexture_bg, new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("texture/turncolour.png")), 50, 50)));
	...
	public static void loadTexture() {
		...
		Map<String, TextureFile> characterFileMap = JsonParser.parseMap(TextureFile.class, filePathMap.get(JsonEnum.CHARACTER_FILE_PATH.toString()).getFile());
		for (Entry<String, TextureFile> entry : characterFileMap.entrySet())
			characterTextureMap.put(entry.getKey(), entry.getValue().getFile());

		Map<String, TextureFile> monsterFileMap = JsonParser.parseMap(TextureFile.class, filePathMap.get(JsonEnum.MONSTER_FILE_PATH.toString()).getFile());
		for (Entry<String, TextureFile> entry : monsterFileMap.entrySet())
			monsterTextureMap.put(entry.getKey(), entry.getValue().getFile());

		Map<String, TextureFile> backgroundFileMap = JsonParser.parseMap(TextureFile.class, filePathMap.get(JsonEnum.BACKGROUND_FILE_PATH.toString()).getFile());
		for (Entry<String, TextureFile> entry : backgroundFileMap.entrySet())
			backgroundTextureMap.put(entry.getKey(), entry.getValue().getFile());
	}
}
