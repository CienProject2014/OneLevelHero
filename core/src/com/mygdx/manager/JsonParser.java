...
public class JsonParser {
	...
	@SuppressWarnings("unchecked")
	public static <T> ObjectMap<String, T> parseObjectMap(Class<T> clazz,
			String jsonString) {
		ObjectMap<String, JsonValue> parsedMap = JSON.fromJson(ObjectMap.class,
				jsonString);

		ObjectMap<String, T> result = new ObjectMap<String, T>();

		for (String key : parsedMap.keys()) {
			result.put(key, JSON.readValue(clazz, parsedMap.get(key)));
		}
