...
public class MusicAssets implements FileAssetsInitializable {
	...
	public void set(Map<String, StringFile> filePathMap) {
		Map<String, MusicFile> musicFileMap = JsonParser.parseMap(MusicFile.class,
				filePathMap.get(JsonEnum.MUSIC_FILE_PATH.toString()).loadFile(assetsManager));
		for (Entry<String, MusicFile> entry : musicFileMap.entrySet()) {
			musicMap.put(entry.getKey(), entry.getValue().loadFile(assetsManager));
		}
		...
		// Battle MusicList
		String battleMusicJsonString = filePathMap.get(JsonEnum.BATTLE_MUSIC_LIST.toString()).loadFile(assetsManager);
		...
		// Moving MusicList
		String movingMusicJsonString = filePathMap.get(JsonEnum.MOVING_MUSIC_LIST.toString()).loadFile(assetsManager);
		...
		// Event MusicList
		String eventMusicJsonString = filePathMap.get(JsonEnum.EVENT_MUSIC_LIST.toString()).loadFile(assetsManager);
